    ...
    begin
        if Est_Vide(Arbre) then 
            ...
            if Est_Vide(Arbre.all.Droite.all.Droite) then
                Arbre.all.Gauche := Arbre.all.Gauche.all.Gauche; -- G G
            else
                Arbre.all.Gauche := Arbre.all.Gauche.all.Droite; -- G D
            end if;
            Free(Supr);
        else 
            Supr := Arbre.all.Droite;
            if Est_Vide(Arbre.all.Droite.all.Droite) then 
                Arbre.all.Droite := Arbre.all.Droite.all.Gauche; -- D G
            else 
                Arbre.all.Droite := Arbre.all.Droite.all.Droite; -- D D
            end if;
            Free(Supr);
        end if;
    end Supprimer_Noeud;
